...
@if($productLinks && $productLinks['has_links'])
<section class="product-links-section py-20">
  <div class="container">
    <div class="product-links"
        x-data="{
          links: {{ Js::from($productLinks['links']) }},
          totalCount: {{ $productLinks['total_count'] }},
          visibleLimit: {{ $productLinks['visible_limit'] }},
          showAll: false,
          textLimits: {},
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
            <template x-for="(link, index) in visibleLinks" :key="link.slug">
                <div class="bg-white rounded-lg p-6 border border-gray-200 hover:shadow-lg transition-all duration-300 cursor-pointer group"
                    ...
                    {{-- Логотип/изображение --}}
                    <div class="mt-auto">
                        <template x-if="link.has_logo && link.logo">
                            <div class="flex justify-center ">
                                <img :src="link.logo.sizes?.medium || link.logo.url"
                                     :alt="link.logo.alt || link.title"
